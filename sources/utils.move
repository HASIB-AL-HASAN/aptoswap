module Aptoswap::utils {
    ...
    public fun add_sma128(sma: &mut WeeklySmaU128, time: u64, value: u128) {
        ...
        if (sma.start_time < time + TIME_INTERVAL_DAY) {
            sma.a0 = sma.a0 + value;
            sma.c0 = sma.c0 + 1;
        }
        else if (sma.start_time < time + (TIME_INTERVAL_DAY * 2)) {
            sma.a1 = sma.a1 + value;
            sma.c1 = sma.c1 + 1;
        }
        else if (sma.start_time < time + (TIME_INTERVAL_DAY * 3)) {
            sma.a2 = sma.a2 + value;
            sma.c2 = sma.c2 + 1;
        }
        else if (sma.start_time < time + (TIME_INTERVAL_DAY * 4)) {
            sma.a3 = sma.a3 + value;
            sma.c3 = sma.c3 + 1;
        }
        else if (sma.start_time < time + (TIME_INTERVAL_DAY * 5)) {
            sma.a4 = sma.a4 + value;
            sma.c4 = sma.c4 + 1;
        }
        else if (sma.start_time < time + (TIME_INTERVAL_DAY * 6)) {
            sma.a5 = sma.a5 + value;
            sma.c5 = sma.c5 + 1;
        }
        else {
